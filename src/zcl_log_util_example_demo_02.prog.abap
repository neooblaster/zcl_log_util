...
TYPES: BEGIN OF ty2_my_log_table         ,
         icon     TYPE alv_icon          ,
         vbeln    TYPE vbeln             ,
         vbelp    TYPE vbelp             ,
...
DATA: lt2_log_table TYPE TABLE OF ty2_my_log_table.

" Declaring reference to ZCL_LOG_UTIL
DATA: lr2_log_util TYPE REF TO zcl_log_util.
...
zcl_log_util=>factory(
  ...
  IMPORTING
    e_log_util  = lr2_log_util
  " Passing our log table
  CHANGING
    c_log_table = lt2_log_table
).
...
DATA: lv2_dummy TYPE string.

MESSAGE e504(vl) INTO lv2_dummy.
lr2_log_util->log( ).

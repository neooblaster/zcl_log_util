...
TYPES: BEGIN OF ty60_my_log_table         ,
         icon     TYPE alv_icon          ,
         vbeln    TYPE vbeln             ,
         vbelp    TYPE vbelp             ,
...
DATA: lt60_log_table TYPE TABLE OF ty60_my_log_table.

" Declaring reference to ZCL_LOG_UTIL
DATA: lr60_log_util TYPE REF TO zcl_log_util.
...
zcl_log_util=>factory(
  ...
  IMPORTING
    e_log_util  = lr60_log_util
  " Passing our log table
  CHANGING
    c_log_table = lt60_log_table
).
...
**& • 4.) Simulating Log Table
**&----------------------------------------------------------------------*
DATA: lt60_bapi_ret_tab TYPE TABLE OF bapiret2 .
DATA: ls60_bapi_ret_tab TYPE          bapiret2 .

" Document 1
ls60_bapi_ret_tab-id     = 'MEPO' .
ls60_bapi_ret_tab-number = '2'    .
ls60_bapi_ret_tab-type   = 'E'    .
APPEND ls60_bapi_ret_tab TO lt60_bapi_ret_tab.
...
"
"
lr60_log_util->log( lt60_bapi_ret_tab ).

" In log we want to display the PO Number foe each lines of BAPI
DATA: lt60_log_table_x TYPE TABLE OF ty60_my_log_table.
DATA: ls60_log_table_x TYPE          ty60_my_log_table.

ls60_log_table_x-vbeln = '4500001189'.
APPEND ls60_log_table_x TO lt60_log_table_x.
ls60_log_table_x-vbeln = '4500001190'.
APPEND ls60_log_table_x TO lt60_log_table_x.
ls60_log_table_x-vbeln = '4500001191'.
APPEND ls60_log_table_x TO lt60_log_table_x.
...
lr60_log_util->merging( lt60_log_table_x ).
